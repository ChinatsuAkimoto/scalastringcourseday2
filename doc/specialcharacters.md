...
<h4>native2ascii</h4>
変換に使用されるのがnative2asciiで、これはコマンドラインから使用できます。native2asciiはJDKに含まれています。

変換|コマンドの例
---|---
通常の文字列 to Unicodeシーケンス|```native2ascii -encoding UTF-8 [inputfile] ([outputfile])```
Unicodeシーケンス to 通常の文字列|```native2ascii -reverse -encoding UTF-8 [inputfile] ([outputfile])```
